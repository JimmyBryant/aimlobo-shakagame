...
function playBGM() {
    const bgmBird = new Howl({
        src: ["/audio/game/flying_book/audio_d27edefa4c.mp3"],
        html5: true,
        loop: true,
        volume: 0.3,
    });

    const bgmBG = new Howl({
        src: ["/audio/game/flying_book/serene-sky-relaxing-piano-176513.mp3"],
        html5: true,
        loop: true,
        volume: 0.5,
    });
...
// 显示birds-box
function showBirdsBox() {
    const birdsBox = document.querySelector('.birds-box');

    if (birdsBox) {
        displayBlock(birdsBox);
    }
}

// 显示book-content: 为了让它显示在页面中间位置，让总体的.game_flying_book变为flex，同时隐藏book_cover
function showBookContent() {
    // const gameFlyingBook = document.querySelector('.game_flying_book');
    ...
    // 获取数据并填充
    varBookContent.textContent = getRandomContent(getLanguageFromUrl());


    // 显示book-content
    if (bookContent) {
        displayFlex(bookContent);
    }
}
...
function startGame() {
    ...
    btnStartGame.addEventListener('click', (event) => {
        ...
        // 记录积分消耗 和 时间: 只有点击这里才算一次游戏次数
        updateGameTime(1);
        updateTotalLuckyPoint(-0.5);
        writeEventLogLuckyPoint('flying book start', -0.5);
        writeEventLogGameTime('flying book', 1);
...
function restartGame() {
    ...
    btnRestartGame.addEventListener('click', (event) => {
        varBookContent.textContent = getRandomContent(getLanguageFromUrl());
        updateTotalLuckyPoint(-0.5);
        writeEventLogLuckyPoint('flying book again', -0.5);
        event.stopPropagation(); // 阻止事件冒泡
    }); 
}
...
function getRandomContent(language) {
    const contents = jsonData[language];
